...
# Extra Chromium flags for the container (e.g. "--no-sandbox --disable-dev-shm-usage").
LAUNCH_ARGS += os.getenv("DEEPSEEK_BROWSER_ARGS", "").split()

# Token is trusted for this long before we refresh it from the browser again.
# In the container the Chrome profile is NOT portable across OSes, so we rely on
# a hand-carried session.json and keep it valid for longer (override via env).
SESSION_MAX_AGE = int(os.getenv("SESSION_MAX_AGE", str(6 * 60 * 60)))  # 6h default
