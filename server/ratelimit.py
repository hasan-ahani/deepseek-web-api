...
def install_rate_limit(
    ...
    *,
    ...
    `key_func(request)` overrides how the caller is identified (defaults to
    client IP). Pass an API-key-based key so all requests from one trusted
    caller share a single bucket instead of being split per proxy IP.
    """
    key_func = key_func or _client_key
