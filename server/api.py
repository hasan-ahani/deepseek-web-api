...
Requests under /v1 require an `Authorization: Bearer <WEUI_AI_API_KEY>` header
and are rate limited per caller (default 30/min, set via RATE_LIMIT_PER_MINUTE);
/healthz is exempt from both.
"""
...
from .security import bearer_token, install_api_key
...
install_rate_limit(
    app,
    RateLimiter(limit=RATE_LIMIT_PER_MINUTE, window=60.0),
    # Bucket by API key so all calls from one trusted caller share a bucket.
    key_func=lambda r: bearer_token(r) or (r.client.host if r.client else "unknown"),
)
...
async def session_status():
    """Report whether a usable DeepSeek session is available (for monitoring)."""
    global _client
    if _client is None:
        try:
            await run_in_threadpool(get_client)
        except LoginRequired as e:
            return {"status": "login_required", "detail": str(e)}
    ...
    return {
        "status": "ok",
        "session_age_seconds": int(session.age) if session else None,
    }
